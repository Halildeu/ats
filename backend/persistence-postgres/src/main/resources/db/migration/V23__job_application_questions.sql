...
-- `[]` = soru YOK; NULL olamaz, çünkü "bilinmiyor" ile "yok" karışırsa aday
-- formu hangi durumda soru göstereceğini bilemez.
...
-- ile veri yükleme) aday formu 200 soruyla açılmasın. Onaylı sözleşme: 0..10
-- geçerli, 11 reddedilir.
ALTER TABLE ats_job_posting
    DROP CONSTRAINT IF EXISTS ats_job_posting_questions_max;
ALTER TABLE ats_job_posting
    ADD CONSTRAINT ats_job_posting_questions_max
    CHECK (jsonb_array_length(questions) BETWEEN 0 AND 10);

COMMENT ON COLUMN ats_job_posting.questions IS
    '#240 A: ilana ozel basvuru sorulari '
    '[{questionId,order,text,kind,required,options:[{optionId,label}]}]. '
    'kind kapali kume: SHORT_TEXT|LONG_TEXT|YES_NO|SINGLE_CHOICE. '
    'questionId/optionId KIMLIK, order yalniz gorunum sirasi. '
