...
- 🔓 Persistence unlock: [ATS-0018](../docs/adr/ATS-0018-persistence-unlock-postgres-jdbc.md) — **PG16 + Flyway + plain-JDBC tek-adapter-modülü (`persistence-postgres`); JPA/Hibernate/Spring-Data KULLANILMAZ**. Durum: **8a worm_ledger + 8b 6-store adapter LANDED**; **8c retention-purge** artık kalıcı cross-plane execution/step defteri, lease/CAS resume ve object-store opak anahtar silmesini kullanır. #169 adayı V7 ile server-authoritative DSAR scope + terminal interview seal/no-resurrection ekler. Receipt sözleşmesinde `deletedContentCount` yalnız ana veri düzlemlerindeki doğrulanmış silme etkisini sayar; object-store çağrısı `objectDeleteIssuedCount` olarak ayrıdır ve kalıcı/crypto-erasure kanıtı değildir (önceki aday makbuzlarında object etkisi bu sayı içinde görünmüş olabilir). Gerçek object-store G0 kararı yokken boot yalnız açık `ATS_OBJECT_STORE_MODE=in-memory-dev` opt-in'iyle mümkündür; bu production-ready/crypto-erasure kanıtı değildir. Prod deploy/DSN/Vault wiring ve #170 Legal/DPO immutable-field kararı hâlâ dış gate'tir.
...
- 🟡 Slice-12 REST: export + DSAR/erasure yüzeyi mevcut. POST dsar/erasure yalnız strict `{dsarKey}` kabul eder; hedef scope caller'dan alınmaz, server truth'undan çözülür. Tombstone/content adımları durable saga ile resume edilir; `ats.dsar.write` intake ve **`ats.erasure.execute` yıkıcı yürütme** ayrı kalır. #169 gerçek-PG/CI kanıtı ve #170 Legal/DPO kararı gelmeden bu satır product/legal kapanış iddiası değildir.
