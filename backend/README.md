...
- 🔓 Persistence unlock: [ATS-0018](../docs/adr/ATS-0018-persistence-unlock-postgres-jdbc.md) — **PG16 + Flyway + plain-JDBC tek-adapter-modülü (`persistence-postgres`); JPA/Hibernate/Spring-Data KULLANILMAZ**. Durum: **8a worm_ledger + 8b 6-store adapter LANDED**; **8c retention-purge** artık kalıcı cross-plane execution/step defteri, lease/CAS resume ve object-store opak anahtar silmesini kullanır. #169 adayı V7 ile server-authoritative DSAR scope + terminal interview seal/no-resurrection ekler; prod deploy/DSN/Vault wiring ve #170 Legal/DPO immutable-field kararı hâlâ dış gate'tir.
