...
public final class ExportService {
    ...
    public Outcome<ExportReceipt> exportPacket(TenantId tenantId, ActorId actorId, InterviewId interviewId,
        ...
        for (String citationKey : citationKeys) {
            ...
            claims.add(JsonValue.object(Map.of(
                    "claim_id", JsonValue.of(citationKey),
                    // statement_ref = silinebilir-düzlem pointer'ı (claim METNİ packet'e GİRMEZ)
                    "statement_ref", JsonValue.of(citationKey),
        ...
        JsonValue.JsonObject packetBody = buildPacket(tenantId, interviewId, caseKey, reviewCase, claims, ctx, occurredAtIso);
    ...
    private JsonValue.JsonObject buildPacket(TenantId tenantId, InterviewId interviewId, String caseKey,
            ReviewCase reviewCase, List<JsonValue> claims, ExportContext ctx, String occurredAtIso) {
        ...
        List<JsonValue> evidenceRefs = reviewCase.sourceEvidenceRefs().stream().map(JsonValue::of).toList();

        Map<String, JsonValue> m = new HashMap<>();
        m.put("schema_version", JsonValue.of(SCHEMA_VERSION));
        m.put("packet_id", JsonValue.of("pkt-" + caseKey));
        ...
        m.put("export_event_ref", JsonValue.of(tenantId.value() + ":" + interviewId.value() + ":export:" + caseKey));
