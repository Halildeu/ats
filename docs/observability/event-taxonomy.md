...
| **evidence.human_approval.recorded** | evidence | notice | id-only | actor_ref | gate-locked |
...
- **Sentinel event ID silme guard'ı:** kritik denetim event'leri (`authz.tenant_boundary.violation`, `admin.breakglass.invoked`, `evidence.tombstone.appended`, `evidence.human_approval.recorded`, `privacy.erasure.executed`, `ai_pipeline.prompt_injection.blocked`) silinemez.
