...
| K2 | m.5/6 — işleme şartı: açık rıza | Rıza DENY-BY-DEFAULT (kayıt yok→red); rıza yazımı state+WORM kanıtı BİRLİKTE (GRANTED=ledger-önce); yeniden-beyan yeni kanıt; **rıza kapısı STT işlemesinde DE zorunlu — geri-çekme ingest-sonrası transkripsiyon dahil derhal durdurur** | `backend/consent/src/main/java/com/ats/consent/ConsentGate.java` · `backend/consent/src/main/java/com/ats/consent/ConsentService.java` · `backend/consent/src/test/java/com/ats/consent/ConsentServiceTest.java` · `backend/ai-orchestration/src/test/java/com/ats/orchestration/TranscriptionServiceTest.java` | enforced (repo-test) |
...
| K6 | m.11 — ilgili kişinin hakları (erişim/bilgi talebi) | Erasure intake yalnız identity-issued UUIDv4 subjectRef + kapalı `DATA_SUBJECT_ERASURE` kodu alır; exact JSON zarfı, domain ve doğrulanmış DB constraint'i serbest metni keser. Erasure ayrı YIKICI yetki sınıfıdır; yeni hukuki kategori #170 Legal/DPO gate'idir. | `backend/app-boot/src/main/java/com/ats/app/web/DsarApiController.java` · `backend/retention-dsr/src/main/java/com/ats/dsr/DsarInputPolicy.java` · `backend/persistence-postgres/src/main/resources/db/migration/V8__dsar_input_contract.sql` · `backend/app-boot/src/test/java/com/ats/app/ExportDsarApiTest.java` | enforced (repo-test); live/legal gate açık |
| K7 | m.12 — veri güvenliği (uygun teknik tedbirler; m.12/3 amaç-dışı kullanım/ifşa yasağı dahil) | JWT+JWKS (iss/aud/exp) + endpoint-bazlı scope ayrımı (**STT işleme = PII-üreten AYRI yetki sınıfı `ats.transcription.write`; kayıt-yükleme yetkisi tek başına yetmez**) + tenant DAİMA token'dan + bilinmeyen yüzey denyAll; **transkripsiyon yalnız `recording.ingested` WORM kanıtı olan kaynak için çalışır (kanıt-zinciri bütünlüğü — sahte kaynak-anahtarıyla kanıt üretimi yapısal imkânsız)**; tehdit-kaydı ve kontrol haritası | `backend/app-boot/src/main/java/com/ats/app/SecurityConfig.java` · `backend/app-boot/src/test/java/com/ats/app/RestApiSecurityTest.java` · `backend/ai-orchestration/src/main/java/com/ats/orchestration/TranscriptionService.java` · `backend/app-boot/src/test/java/com/ats/app/CitationReviewApiTest.java` · `backend/ops-events/src/main/java/com/ats/ops/PiiClass.java` · `docs/security/threat-register.md` · `docs/security/control-map.md` | enforced (repo-test) |
...
| K9 | Özel nitelikli veri (m.6) — ses/biyometri sınırı | Diarization=AYRIŞTIRMA (session-scoped S1..Sn; kalıcı şablon YOK); sesten-kimlik default-DIŞLANMIŞ (opt-in yalnız iç-kullanıcı, aday kategorik-dışlanmış, çift-kilit); **runtime karşılığı: SegmentSanitizer STT çıktısında konuşmacıyı S1..Sn takma-ada çevirir ve paralinguistik/affect işaretlerini ([alkış], parantez-içi) transkripte SOKMAZ — WORM payload'ına affect-proxy alan yazılmaz** | `docs/ai-governance/speaker-attribution-standard.md` · `docs/privacy/dpia-voice-enrollment.md` · guard `scripts/check-speaker-attribution.mjs` · `backend/ai-orchestration/src/main/java/com/ats/orchestration/SegmentSanitizer.java` · `backend/ai-orchestration/src/test/java/com/ats/orchestration/TranscriptionServiceTest.java` | enforced (repo-test) |
