...
| K6 | m.11 — ilgili kişinin hakları (erişim/bilgi talebi) | DSAR intake API'si (opak subjectRef; talep WORM/ops izli); erasure ayrı YIKICI yetki sınıfı | `backend/app-boot/src/main/java/com/ats/app/web/DsarApiController.java` | enforced (repo-test) |
| K7 | m.12 — veri güvenliği (uygun teknik tedbirler) | JWT+JWKS (iss/aud/exp) + endpoint-bazlı scope ayrımı + tenant DAİMA token'dan + bilinmeyen yüzey denyAll; tehdit-kaydı ve kontrol haritası | `backend/app-boot/src/main/java/com/ats/app/SecurityConfig.java` · `docs/security/threat-register.md` · `docs/security/control-map.md` | enforced (repo-test) |
| K8 | m.12/3 — işleme faaliyet kayıtları/denetlenebilirlik | Append-only WORM ledger (hash-chain; UPDATE/DELETE/TRUNCATE reject-trigger); operasyonel event taksonomisi (pii_class fail-closed) | `backend/persistence-postgres/src/main/resources/db/migration/V1__worm_ledger.sql` · `docs/observability/event-taxonomy.md` | enforced (repo-test) |
