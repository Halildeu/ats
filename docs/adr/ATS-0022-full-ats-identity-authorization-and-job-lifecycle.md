...
2026-07-18 canonical inceleme sırasının ilk turunda doğrudan Anthropic CLI ile exact `claude-opus-4-8`, content-addressed ve secret-taramalı exact-head kapsamına `REVISE` verdi. Eski pod'un `published=true` yazısıyla terminal ilanı yeniden yayınlayabilmesi veya aktif kariyer sitesi olmadan yayın oluşturabilmesi P1 olarak; public yol alias'larının ayrı rate-limit bucket'ları, recruiter mutasyonlarında pre-deserialization body limiti eksikliği ve tenant-geneli idempotency namespace'inin açık belgelenmemesi P2 olarak raporlandı. Sonraki exact-head Claude turu P0/P1 olmadan `AGREE` verdi; kalan P2 migration backfill, replay timestamp biçimi ve kanonik job-manage/publish yetki semantiği de absorbe edildi.

MiniMax M3'ün sonraki turları pre-V7 yayınların güvenli karantinaya alınması, inactive-site alias'larının fail-closed davranması, PDP URL'sinin kanonikleştirilmesi, OpenAPI sınırları, replay başlıkları ve operasyon runbook'u için `REVISE` verdi; bunlar absorbe edildi. Son turdaki legacy audit digest'inin içerik değişikliklerini ayırt edememesi ve kariyer sitesi deactivation/handle-rotation operasyonunun eksikliği de gerçek bulgu kabul edilerek düzeltildi. Token'sız recruiter çağrısının 401 yerine NPE/500 verdiği, direct SQL publish'in site invariant'ını aşabildiği ve PostgreSQL `ON CONFLICT DO NOTHING` yarışında meşru yarım satır oluştuğu iddiaları ise entegrasyon testleri ve PostgreSQL conflict-wait semantiğiyle karşılanarak doğrulanmadı. Özellikle eşzamanlı idempotency iddiası için kazanan transaction'ı job insert aşamasında geciktiren iki-thread PostgreSQL 16 testi eklendi; ikinci istek 5xx/yarım snapshot değil exact `REPLAYED` sonucu alır ve tek event üretilir. Gereksiz `FOR UPDATE` veya transaction dışı ikinci okuma eklenmedi. Bu değişiklikler yeni head ürettiği için Claude Opus 4.8 -> MiniMax M3 -> Codex 5.6 SOL incelemesi aynı yeni kapsam üzerinde yeniden tamamlanmadan dal merge-ready sayılmaz.
