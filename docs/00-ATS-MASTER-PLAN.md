...
## Değişiklik kaydı
- **v3.1 (2026-06-23):** G0 kit 8-doküman (gate+ICP+LOI+scope-freeze+execution-system+pilot-open-checklist+**one-pager+outreach-templates**) + cross-AI gate-lock (Codex REVISE→AGREE, 7 false-GO deliği kapatıldı, thread 019ef313) → [G0/](./G0/). **Foundational ADR seti TAM ATS-0001..0007** cross-AI Accepted (Codex thread 019ef3d9, çok-tur REVISE→AGREE; +0002 tenant-izolasyon, +0006 sovereign-SKU-gated, +0007 security/supply-chain) → [adr/](./adr/). Pilot-open release checklist **Gate A-F**. **Eval-harness** (ATS-0004 Gate C ölçüm rig'i: WER/DER/citation/fail-closed, 11/11 test pass, fail-closed doğrulandı) → [eval-harness/](./eval-harness/). Issue ats#1 In Progress, ats#3 Needs Verify.
