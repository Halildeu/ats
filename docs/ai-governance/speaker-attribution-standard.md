...
- **status:** `active-compliant` (biyometrisiz, izinli) · `active-internal-consented` (**design-plane**: owner risk-beyanı KAYITLI — [[ATS-0014]]; YALNIZ iç-kullanıcı opt-in, aday daima dışlanmış; **fiili rıza toplandı anlamına GELMEZ** — runtime-enable = fiili açık-rıza + imzalı-DPIA + VERBIS + P1 çift kilit) · `excluded-biometric` (beyan-öncesi default; geri dönüş = ADR+guard değişikliği).
...
4. Sentinel `voiceprint_enrollment` §2'de **durmalı** (status = `active-internal-consented`; owner beyanı [[ATS-0014]]) + hiçbir zaman §1'e taşınamaz + **koşul/scope hücresi LITERAL-PİNLİ** (herhangi bir ekleme/çıkarma — örn. aday-enrollment genişletmesi, "imza beklemeden/hemen kullanılabilir" runtime-dili — guard'ı kırar; bilinçli değişiklik = ADR + guard birlikte) + iç-kullanıcı / aday-ASLA (candidate_exclusion) / amaç-sınır / self-service / consent-texts+dpia linkleri / DPIA / P1 tokenları zorunlu.
...
|---|---|---|---|
| **voiceprint_enrollment** | kalıcı ses-şablonu (voiceprint) + kimlik eşleme = KVKK m.6 **özel-nitelikli biyometrik** (açık rıza şart) + GDPR Art.4(14)/Art.9 biyometrik eşiği + EU AI Act **biyometrik-tanımlama yüksek-risk** + US eyalet-hukuku maruziyeti (Illinois BIPA vb.; jurisdictional). Ağır rejim — bu yüzden scope DAR tutulur | **owner design-plane risk-beyanı KAYITLI** (2026-07-02, [[ATS-0014]] Accepted): YALNIZ **iç-kullanıcı** opt-in; **aday ASLA enroll edilmez** (candidate_exclusion; eleme-yoluyla öneri); **amaç-sınırlı** (yalnız attribution önerisi — authentication/izleme yasak) + **self-service silme** (crypto-erase, [[data-lifecycle-register]] `voiceprint_template`); rıza-metni + DPIA **TASLAKLARI landed** ([[consent-texts-voice-enrollment]], [[dpia-voice-enrollment]]) — **fiili açık-rıza toplama + tenant yayımı + DPO/owner imzası + VERBIS = runtime-enable önkoşulu**; runtime **P1** gate-locked | active-internal-consented |

## 3. Doğrulama (`scripts/check-speaker-attribution.mjs`)

§0 sözlük + invariant 1–6: aktif yöntem allowlist (yalnız 4, duplicate yasak, strict satır formatı) + `biometric=no` + yasak biyometrik-kavram taraması (§1 gövdesinin tamamı; TR alias'lar dahil); insan-onay tokeni (lexical/human_labeling); sentinel voiceprint_enrollment korunur + §1'e taşınamaz + status=active-internal-consented + **koşul hücresi literal-pin** (çelişkili genişletme/runtime-dili eklenirse fail) + scope-token seti (consent-texts+dpia linkleri dahil); embedding invariant cümlesi literal-pinli; bölüm yapısı allowlist; cross-doc: event-taxonomy `evidence.speaker.attributed`+`ledger_entry_ref` + data-lifecycle `speaker_attribution_map` + **`voiceprint_template` satır-tokenları** (gate-locked/İNERT/runtime-P1/imzalı-DPIA/aday-ASLA/crypto-erase/transfer-none); gömülü self-test (negatif vektörler fail eder).
